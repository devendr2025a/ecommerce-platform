...
import { Plus, Edit2, Trash2, X, Upload, Search, Package, Image as ImageIcon, AlertCircle } from 'lucide-react';
...
export default function AdminProducts() {
  ...
  const handleFileChange = (e) => {
    ...
    if (files.length > remaining) {
      toast.error(`You can add at most ${remaining} more image${remaining !== 1 ? 's' : ''}`);
      return;
    }
  ...
  /* ─── Render ─── */
  const activeExisting = existingImages.filter((img) => !imagesToDelete.includes(img._id));
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-gray-600 hidden lg:table-cell">Images</th>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                ...
                <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  className="input-field text-sm" placeholder="Product name" />
              </div>
              ...
              <div>
                ...
                <textarea required rows={3} value={form.description}
                  ...
                  className="input-field text-sm resize-none" placeholder="Product description" />
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-xs font-semibold ${totalSlots >= 5 ? 'text-red-500' : 'text-gray-400'}`}>
                    {totalSlots}/5 used
                  </span>
                </div>
                ...
                {existingImages.length > 0 && (
                  <div className="mb-3">
                    <p className="text-xs text-gray-500 mb-2 font-medium uppercase tracking-wide">Current Images</p>
                    <div className="flex flex-wrap gap-2">
                      {existingImages.map((img) => {
                        const url = getImageUrl(img);
                        const markedForDelete = imagesToDelete.includes(img._id);
                        return (
                          <div key={img._id} className={`relative group w-20 h-20 rounded-lg overflow-hidden border-2 transition-all
                            ${markedForDelete ? 'border-red-400 opacity-40' : 'border-gray-200'}`}>
                            {url
                              ? <img src={url} alt="" className="w-full h-full object-cover" />
                              : <div className="w-full h-full bg-gray-100 flex items-center justify-center"><ImageIcon className="h-5 w-5 text-gray-400" /></div>
                            }
                            ...
                            >
                              {deletingImageId === img._id
                                ? <span className="text-white text-[10px] font-bold">Removing…</span>
                                : markedForDelete
                    ...
                    <p className="text-[11px] text-gray-400 mt-1.5">Hover over an image and click the trash icon to remove it.</p>
                  </div>
                )}
                ...
                {newPreviews.length > 0 && (
                  <div className="mb-3">
                    <p className="text-xs text-gray-500 mb-2 font-medium uppercase tracking-wide">New Images to Upload</p>
                    <div className="flex flex-wrap gap-2">
                      {newPreviews.map((url, idx) => (
                        <div key={idx} className="relative group w-20 h-20 rounded-lg overflow-hidden border-2 border-blue-300">
                          <img src={url} alt="" className="w-full h-full object-cover" />
                          <button
                            type="button"
                ...
                {totalSlots < 5 && (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-500">
                      Click to {existingImages.length > 0 ? 'add more' : 'upload'} images
                    </p>
                ...
                {totalSlots >= 5 && (
                  <div className="flex items-center gap-2 text-amber-600 bg-amber-50 border border-amber-200 rounded-lg p-3">
                    ...
                    <p className="text-xs">Maximum 5 images reached. Remove an existing image to add a new one.</p>
                  </div>
                )}

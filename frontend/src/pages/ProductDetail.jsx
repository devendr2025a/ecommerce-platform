          ...
          <div className="space-y-8">
            ...
            <div className="py-6 border-t border-b border-[var(--vg-border)] space-y-5">
              ...
              <div className="bg-[var(--vg-gray)] border border-[var(--vg-border)] p-4 flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-black text-[var(--vg-black)] uppercase tracking-widest">
                    Limited Time Offer
                  </p>
                  <p className="text-[10px] text-[var(--vg-muted)] font-bold uppercase tracking-widest mt-0.5">
                    Sale expires soon. Act now.
                  </p>
                </div>
                <div className="flex gap-3">
                  {["02", "14", "55"].map((v, i) => (
                    <div key={i} className="flex flex-col items-center">
                      <span className="text-base font-black text-[var(--vg-black)]">
                        {v}
                      </span>
                      <span className="text-[8px] text-[var(--vg-muted)] font-bold uppercase tracking-wider">
                        {["HRS", "MIN", "SEC"][i]}
                      </span>
                    </div>
                  ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-[var(--vg-gray)] border border-[var(--vg-border)] p-4 space-y-1">
                <span className="text-[9px] font-black text-[var(--vg-muted)] uppercase tracking-widest">
                  Fabric
                </span>
            ...
            <div className="flex items-center gap-4 bg-[var(--vg-gray)] border-l-4 border-[var(--vg-black)] p-4">
              <div>
                <p className="text-[9px] font-black text-[var(--vg-muted)] uppercase tracking-widest mb-0.5">
                  Model Reference
                </p>
              ...
              {[
                {
                  title: "Fabric & Care",
                  content:
                    "Gentle machine wash cold. Do not bleach. Tumble dry low. Cool iron if needed. 95% Organic Cotton, 5% Elastane.",
                },
                {
                  title: "Size Guide",
                  content:
                    "Sizes follow international standards. For a relaxed fit, size up. Model is 6'1\" (185cm) wearing size Large.",
                },
                {
                  title: "Shipping & Returns",
                  content:
                    "Dispatched within 24–48 hours. 7-day return policy. Quality assured on every shipment.",
                },
              ].map((item, idx) => (
                <details
                  key={idx}
                  className="group border-b border-[var(--vg-border)]"
                >
                  <summary className="flex items-center justify-between py-4 cursor-pointer list-none">
                    <span className="text-[11px] font-black text-[var(--vg-black)] uppercase tracking-[0.2em]">
                      {item.title}
                    </span>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${product.stock > 0 ? (product.stock < 10 ? "bg-orange-500 animate-pulse" : "bg-green-500") : "bg-red-500"}`}
                />
                <span
                  className={`text-[11px] font-bold uppercase tracking-widest ${product.stock > 0 ? (product.stock < 10 ? "text-orange-500" : "text-[var(--vg-muted)]") : "text-red-500"}`}
                >
                  {product.stock > 0
                    ? product.stock < 10
                ...
                <div className="space-y-3">
                  ...
                  <div className="fixed bottom-0 left-0 right-0 z-[90] lg:hidden bg-white border-t border-[var(--vg-border)] px-4 py-3 flex items-center gap-3">
                    <div className="flex items-center border border-[var(--vg-border)]">
                      <button
                        onClick={() => setQuantity(Math.max(1, quantity - 1))}
                        className="px-3 py-2.5 hover:bg-[var(--vg-gray)] transition-colors"
                      >
                        <Minus className="h-3.5 w-3.5 text-[var(--vg-black)]" />
                      </button>
                      <span className="px-4 text-sm font-black text-[var(--vg-black)]">
                        {quantity}
                      </span>
                      <button
                        onClick={() =>
                    ...
                    <button
                      onClick={handleAddToCart}
                      className="btn-primary flex-1 py-3 text-[11px]"
                    >
                      Add to Bag · ₹{discountedPrice.toLocaleString("en-IN")}
                    </button>
                  </div>
              ...
              {product.reviews?.length > 0 ? (
                product.reviews.map((r) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-[11px] font-black text-[var(--vg-black)] uppercase tracking-wider mb-1">
                        {r.name}
                      </p>
                      <p className="text-[var(--vg-muted)] text-sm leading-relaxed">
                        "{r.comment}"
            ...
            {user && (
              <div className="lg:col-span-5 bg-[var(--vg-gray)] border border-[var(--vg-border)] p-8 h-fit sticky top-28">
                ...
                <form onSubmit={handleSubmitReview} className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-[var(--vg-muted)] uppercase tracking-widest">
                      Rating
                    </label>
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map((s) => (
                        ...
                        >
                          <Star
                            className={`h-7 w-7 transition-all ${s <= review.rating ? "fill-[var(--vg-black)] text-[var(--vg-black)]" : "fill-none text-gray-300 hover:text-[var(--vg-black)]"}`}
                          />
                        </button>
                      ))}
                    </div>

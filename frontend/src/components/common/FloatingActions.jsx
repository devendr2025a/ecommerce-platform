...
export default function FloatingActions() {
  ...
  const cartIconImg = "/images/blinkit/widget_cart_icon.png";
  const heartIconImg = "/images/blinkit/widget_heart_icon.png";
  ...
  return (
    <aside
      ...
      className="fixed right-2 sm:right-3.5 bottom-5 sm:bottom-6 z-50 flex flex-col items-center gap-2 select-none"
    >
      {/* ─────────────────────────────────────────────────────────────
          COMPACT ULTRA-PREMIUM NEUMORPHIC FLOATING PILL DOCK
         ───────────────────────────────────────────────────────────── */}
      <div
        className="w-[46px] sm:w-[50px] py-2.5 sm:py-3 px-1 rounded-[26px] sm:rounded-[28px] bg-white/95 backdrop-blur-md border border-blue-50/80 flex flex-col items-center gap-2 sm:gap-2.5 transition-all duration-300 shadow-[0_10px_28px_-4px_rgba(20,70,160,0.16),0_0_20px_2px_rgba(220,238,255,0.7)]"
      >
        {/* 1. CART ACTION BUTTON */}
        <div className="relative group flex items-center">
          {/* Slide-out Tooltip */}
          <div className="absolute right-full mr-3 opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 transition-all duration-200 pointer-events-none z-50">
            <div className="bg-gray-900/90 backdrop-blur-xs text-white text-[11px] font-bold py-1 px-2.5 rounded-lg whitespace-nowrap shadow-lg flex items-center gap-1.5">
              <span>Shopping Cart</span>
              {cartCount > 0 && (
                <span className="bg-[#008848] text-white text-[9px] px-1.5 py-0.2 rounded-full">
                  {cartCount}
                </span>
              )}
            </div>
          ...
          <Link
            to="/cart"
            className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center transition-all duration-200 transform hover:scale-110 active:scale-95 cursor-pointer"
          ...
          >
            ...
            />
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-[#008848] text-white text-[9px] font-black min-w-[17px] h-[17px] px-1 rounded-full flex items-center justify-center shadow-xs border-[1.5px] border-white animate-pulse">
                {cartCount}
              </span>
            )}
          </Link>
        ...
        <div className="relative group flex items-center">
          ...
          >
            ...
            />
            {wishlist?.length > 0 && (
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] font-black min-w-[17px] h-[17px] px-1 rounded-full flex items-center justify-center shadow-xs border-[1.5px] border-white">
                {wishlist.length}
              </span>
            )}
          </Link>
        ...
        <div className="relative group flex items-center">
          {/* Slide-out Tooltip */}
          <div className="absolute right-full mr-3 opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 transition-all duration-200 pointer-events-none z-50">
            <div className="bg-[#128C7E] text-white text-[11px] font-bold py-1 px-2.5 rounded-lg whitespace-nowrap shadow-lg flex items-center gap-1">
      ...
      {/* ─────────────────────────────────────────────────────────────
          SCROLL TO TOP BUTTON (Compact Micro-action with Tooltip)
         ───────────────────────────────────────────────────────────── */}
      {showScrollTop && (
        <div className="relative group flex items-center animate-in fade-in zoom-in">
          {/* Slide-out Tooltip */}
          <div className="absolute right-full mr-3 opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 transition-all duration-200 pointer-events-none z-50">
            <div className="bg-gray-900/90 backdrop-blur-xs text-white text-[11px] font-bold py-1 px-2.5 rounded-lg whitespace-nowrap shadow-lg">
          ...
          <button
            type="button"
            onClick={scrollToTop}
            className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-white/95 backdrop-blur-md text-gray-700 hover:text-[#008848] border border-blue-50/80 shadow-md flex items-center justify-center transition-all duration-200 transform hover:scale-110 active:scale-90 cursor-pointer"
            style={{
